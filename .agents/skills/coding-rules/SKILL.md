...
- **Light & Dark Theme Adaptability (ライトモード・ダークモード両対応)**:
  - 新規UIコンポーネント（ダイアログ、ボトムシート、設定カード、各種シート等）を作成・改修する際は、特定テーマ向けの色をハードコード（色コード直指定）してはならない。
  - 必ず `AppColors` の動的セマンティックカラー（`AppColors.bgElevated`, `AppColors.textPrimary`, `AppColors.textSecondary`, `AppColors.border` 等）を使用する。
  - テーマごとに透明度やシャドウ（`BoxShadow`）を個別に調整したい場合は、`AppColors.isDark`（または `Theme.of(context).brightness`）を用いた条件分岐を使い、ライトモードとダークモードの両方で可読性とデザインの一貫性を保証すること。
